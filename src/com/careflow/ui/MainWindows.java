...
public class MainWindows extends BaseFrame
{
  ...
  private MainWindows()
  {
    String title = new String("Careflow ERP" + Version.getInstance().getVersion() + "                                  Welcome to CareFlow ERP System!");
    try {
    ...
    if (SystemTray.isSupported()) {
      ...
      try
      {
        trayIcon1 = new TrayIcon(ImageManager.getImageIconByShortName("yygl.png").getImage(), "Careflow ERP", this.popup);
      }
      catch (Exception e1) {
        e1.printStackTrace();
      }
